...
if (pf) {
  pf.addEventListener('submit', async (e) => {
    ...
    try {
      const cats = $$('#professionalCategories .chip.active')
        .map(x => x.dataset.slug);

      const data = {
        name: $('#proName').value,
        phone: $('#proPhone').value,
        whatsapp: $('#proPhone').value,

        pin: $('#proPin').value,
        pin_confirm: $('#proPinConfirm').value,

        professional_type: $('#proType').value,

        city: $('#proCity').value,
        zone: $('#proZone').value || null,

        category_slugs: cats,

        description: $('#proDescription').value || null
      };

      const j = await postJSON('/api/professionals', data);

      const box = $('#professionalResult');

      box.classList.remove('hidden');

      box.innerHTML = `
        <strong>Regjistrimi u krye me sukses.</strong><br>
        ID profesionisti: ${j.professional_id}<br>
        Statusi: Në pritje të verifikimit<br>
        Founding Member: ${j.founding_member ? 'Po' : 'Jo'}
      `;

      toast('Biznesi u regjistrua dhe pret verifikimin');

      pf.reset();

      $$('#professionalCategories .chip').forEach((chip, index) => {
