...
def home(request: Request):
    ...
    try:
        ...
        return templates.TemplateResponse(request, "index.html", {"categories": cats})
    finally: db.close()
...
def create_request(data: RequestCreate):
    ...
    try:
        ...
        if not user:
            ...
            print("DEBUG DB CATEGORIES:", [(c.id, c.name, c.slug, c.active) for c in db.query(Category).all()], flush=True)
